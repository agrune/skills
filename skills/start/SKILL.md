---
name: agrune:start
description: agrune 원스톱 온보딩. "agrune 시작", "agrune 셋업", "agrune 설치", "시작", "start", "setup" 등을 말하면 이 스킬을 사용할 것. 환경 검증, 어노테이션, Chrome 확장 설치까지 한 번에 처리한다.
...
어노테이션이 완료된 상태에서 `agrune_sessions` MCP 도구를 호출하여 브라우저 연결을 확인한다.

**연결 성공:** Step 6으로 진행.

**연결 실패:** Chrome 확장이 설치되지 않았거나, Chrome이 실행되지 않았거나, 페이지가 아직 새로고침되지 않은 상태. 다음을 수행한다:
...
4. 사용자가 준비됐다고 하면 `agrune_sessions`를 다시 호출하여 확인한다. 어노테이션이 이미 적용된 상태이므로 페이지만 열려 있으면 성공한다.
5. 여전히 실패하면 트러블슈팅 안내:
